...
async def worker(name, queue, results):
    """ A function to take unmake requests from a queue and perform the work
...
async def distribute_work(url, requests, conncurrency, results):
    ...
    print(
        f"{conncurrency} workers took {total_time:.2f} second to complete {len(results)}"
    )


def assault(url, requests, conncurrency):
    """ Entrypoint to making requests """
    results = []
    asyncio.run(distribute_work(url, requests, conncurrency, requests))
    print(results)
